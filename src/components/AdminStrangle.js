...
const AdminStrangle = ({ filterOptionChainList, height }) => {
  ...
  useEffect(() => {
    if (
      filterOptionChainList?.length > 0 &&
      ...
      setTempStraddleData((prevStraddleData) => {
        ...
        const mergedData = updatedChainData.map((data) => ({
          ...data,
          celot:
            prevStraddleData.find(
              (d) => d.instrumentToken === data.instrumentToken
            )?.celot || data.celot,
          celotqty:
            prevStraddleData.find(
              (d) => d.instrumentToken === data.instrumentToken
            )?.celotqty || data.celotqty,
          pelot:
            prevStraddleData.find(
              (d) => d.instrumentToken === data.instrumentToken
            )?.pelot || data.pelot,
  ...
  useEffect(() => {
    if (tempStraddleData?.length > 0 && strikePrices > 0) {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        let ceData = tempStraddleData.find(
          (data) =>
            data.strikePrice === callvalue.toString() &&
            data.instrumentType === "CE"
        );
        let peData = tempStraddleData.find(
          (data) =>
            data.strikePrice === putcalvalue.toString() &&
            data.instrumentType === "PE"
        );
        if (ceData != null) {
          setStrangleList((previousData) => {
            if (previousData != null) {
              let isPresent = previousData?.find(
                (data) =>
                  data.instrumentType === "CE" &&
                  data.strikePrice === ceData.strikePrice
              );
              if (isPresent !== null && isPresent !== undefined) {
                const updatedData = previousData.map((item) => {
                  if (
                    item.strikePrice === ceData.strikePrice &&
                    item.instrumentType === "CE"
                  ) {
                    return {
                      ...item,
          ...
          setStrangleList((previousData) => {
            if (previousData != null) {
              let isPresent = previousData?.find(
                (data) =>
                  data.instrumentType === "PE" &&
                  data.strikePrice === peData.strikePrice
              );
              if (isPresent !== null && isPresent !== undefined) {
                const updatedData = previousData.map((item) => {
                  if (
                    item.strikePrice === peData.strikePrice &&
                    item.instrumentType === "PE"
                  ) {
                    return {
                      ...item,
  ...
  const handdleTextBoxEvent = (e, index, refType) => {
    ...
    setStrangleList((prevRowData) => {
      ...
      if (refType === "celot") {
        ...
        if (selectedValue > 0) {
          let currentTotalQty =
            parseInt(selectedValue) * defaultLotSize + parseFloat(petotalqty);
          let currentTotalLot = parseInt(selectedValue) + parseFloat(pelot);
          prevRowData[index][refType] = selectedValue;
          prevRowData[index]["celotqty"] =
            parseInt(selectedValue) * defaultLotSize;
          ...
          let currentTotalQty =
            parseInt(0) * defaultLotSize + parseFloat(petotalqty);
        ...
        if (selectedValue > 0) {
          let currentTotalQty =
            parseInt(selectedValue) * defaultLotSize + parseFloat(cetotalqty);
          let currentTotalLot = parseInt(selectedValue) + parseFloat(celot);
          prevRowData[index][refType] = selectedValue;
          prevRowData[index]["pelotqty"] =
            parseInt(selectedValue) * defaultLotSize;
          // prevRowData[index]["totallotqty"]=currentTotalQty;
          // prevRowData[index]["totallot"]=currentTotalLot;
        } else {
          ...
          let currentTotalQty =
            parseInt(0) * defaultLotSize + parseFloat(cetotalqty);
  ...
  const handdleOrderInformationForCombineLive = (e, data, pedata, side) => {
    ...
    dataNewStrick = Constant.GetNewStrike(
      globleSymbol,
      pedata.strikePrice,
      temptype,
      strikePriceDiff
    ...
    newFirstInInstrumentToken = Constant.GetStrikeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newFirstInStrike1,
    ...
    newSecondInInstrumentToken = Constant.GetStrikeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newSecondInStrike1,
    ...
    newFirstOutInstrumentToken = Constant.GetStrikeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newFirstOutStrike1,
    ...
    newSecondOutInstrumentToken = Constant.GetStrikeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newSecondOutStrike1,
    ...
    newFirstInExchangeToken = Constant.GetStrikeExchangeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newFirstInStrike1,
    ...
    newSecondInExchangeToken = Constant.GetStrikeExchangeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newSecondInStrike1,
    ...
    newFirstOutExchangeToken = Constant.GetStrikeExchangeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newFirstOutStrike1,
    ...
    newSecondOutExchangeToken = Constant.GetStrikeExchangeToken(
      globleOptionChainList,
      globleSymbol,
      globleExpityvalue,
      newSecondOutStrike1,
      ...
      updatedRows.forEach((row) => {
        ...
        if (parseInt(clonedRow.totalorderqty) >= parseInt(defaultSliceQty)) {
          ...
          clonedRow.nooforderlot = (
            parseInt(defaultSliceQty) / parseInt(defaultQty)
          ).toString();
        } else {
          clonedRow.orderqty = row.totalorderqty.toString();
          clonedRow.nooforderlot = (
            parseInt(row.totalorderqty) / parseInt(defaultQty)
          ).toString();
        }
        scopedUpdatedRowsArray.push(clonedRow);
        row.totalorderqty = Math.max(
          0,
          parseInt(row.totalorderqty) - parseInt(defaultSliceQty)
        );
      });
    }
                ...
                <Table className="align-items-center">
                  <thead className="thead-light">
                    <tr class="text-center">
                      <th
                        colSpan={5}
                        className="bg bg-success text-white call-light"
                        scope="col"
                      >
                        CALL
                      </th>
                      <th
                        className="bg bg-danger  text-white put-light"
                        colSpan={5}
                        scope="col"
                      >
                        PUT
                      ...
                      <th
                        className="bg bg-info text-white combined-light"
                        colSpan={5}
                        scope="col"
                      >
                        COMBINED
