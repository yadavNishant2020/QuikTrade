...
const AdminOrderPositionDetails = ({filterOrderPositionList}) => {
    ...
    useEffect(()=>{ 
      ...
      useEffect(()=>{ 
        let totalMTM=0;        
        const tokensToFilter = globleOrderPosition.map(item => item.instrumentToken);        
  ...
  const calculateUnrealisedPnl = (position, infodata) => {         
    return position.positionsidetype.toLowerCase() === 'buy'
      ?  (parseFloat(infodata.ltp) - parseFloat(position.buyavg)) * (parseFloat(position.buyqty) - parseFloat(position.sellqty))
      :  (parseFloat(position.sellavg) - parseFloat(infodata.ltp)) * (parseFloat(position.sellqty) - parseFloat(position.buyqty));
  };
...
const handdleMoveInOutQtyChange = (e, index,data) => {
  debugger;
    ...
    setOrderPosition((prevRowData) => {
            ...
            if (i === index) {
                ...
                if (parseInt(positionnetlot) < parseInt(selectedValue)) {
                    ...
                    if (!position.alertDisplayed) {
                        alertify.error("Maximum allowed lot size is " + parseInt(positionnetlot).toString());
                    }
                ...
                return {
                    ...
                    moveinouttotalqty:updatedExtqty===""?"0":parseInt(updatedExtqty)*parseInt(defaultSaveedQty),
            ...
            const updatedTempOrderPosition = prevRowData.map((position, i) => {
                if (i === index) {                                      
                  ...
                  return {
                    ...
                    newaddtotalqty:updatedExtqty===""?"0":parseInt(updatedExtqty)*parseInt(defaultSaveedQty),
                    manualExitQtyChange: true,
                  };
                }
                return position;
            ...
            const updatedTempOrderPosition = prevRowData.map((position, i) => {
                if (i === index) {                                      
                  ...
                  return {
                    ...
                    exittotalqty:updatedExtqty===""?"0":parseInt(updatedExtqty)*parseInt(defaultSaveedQty),
                    manualExitQtyChange: true,
                  };
                }
                return position;
      ...
      const ProcessReverseOrderExistLive=(dataInfo)=>{         
        ...
        let configInformation=configData.find((data)=>data.instrumentname===dataInfo.positioninstrumentname && data.expirydate===dataInfo.positionexpirydate && data.clientId===globleSelectedClientInfo);
        ...
        let orderprice=(
          positiontype===undefined?'MKT': positiontype)==='MKT'? dataInfo.ltp:
              ...
              let currentLotQty=(parseInt(dataInfo.positionnetlot)<0?(-1)*parseInt(dataInfo.positionnetlot):parseInt(dataInfo.positionnetlot))
        ...
        let orderprice=(
          positiontype===undefined?'MKT': positiontype)==='MKT'? dataInfo.ltp:
            ...
            let data={
                ...
                instrumentname:dataInfo.positioninstrumentname,             
                orderside:dataInfo.positionordertype,
                orderqty: ((getSetting(dataInfo.positioninstrumentname, dataInfo.positionexpirydate)!=null?parseInt(getSetting(dataInfo.positioninstrumentname, dataInfo.positionexpirydate).defaultQty) : defaultQty) * parseInt(dataInfo.newqty)).toString(),
    ...
    const handleKeyDown = (e) => {
      ...
      const processExitAllPositionLive=()=>{
            ...
            //let defaultLMTPer=getSetting(dataInfo.positioninstrumentname, dataInfo.positionexpirydate)?.defaultLMTPerCentage;            
            const newOrderPosition = filterOrderPosition.map(({ strikeprice, positionproductname, positiontype, positioninstrumentname, positionexpirydate, positionordertype, exitqty, maxorderqty, defaultlotqty, positionsidetype, instrumentToken, positionexchangetoken, ltp,
                ...
                orderqty:"0",
                totalorderqty: ((getSetting(positioninstrumentname, positionexpirydate)!=null?parseInt(getSetting(positioninstrumentname, positionexpirydate).defaultQty) : defaultlotqty) * parseInt(exitqty)).toString(),
            ...
            while (updatedRows.some((row) => row.totalorderqty > 0)) {
                updatedRows.forEach((row) => {   
                  const clonedRow = { ...row }; 
                  if(parseInt(clonedRow.totalorderqty)>= parseInt(clonedRow.maxorderqty)){              
                    clonedRow.orderqty=clonedRow.maxorderqty.toString();
                    clonedRow.nooforderlot=(parseInt(clonedRow.maxorderqty)/parseInt(clonedRow.lotsize)).toString();
                  }else{
                    clonedRow.orderqty=row.totalorderqty.toString()
                    clonedRow.nooforderlot=(parseInt(row.totalorderqty)/parseInt(clonedRow.lotsize)).toString();
                  }  
                  scopedUpdatedRowsArray.push(clonedRow);
                  row.totalorderqty = Math.max(0, parseInt(row.totalorderqty) - parseInt(clonedRow.maxorderqty));        
                });          
          }
      ...
      const processExitAllPositionPaper=()=>{
        ...
        const newOrderPosition = filterOrderPosition.map(({ strikeprice, positionproductname, positiontype, positioninstrumentname, positionexpirydate, positionordertype, exitqty, maxorderqty, defaultlotqty, positionsidetype, instrumentToken, positionexchangetoken, ltp,
            ...
            orderstatus:((positiontype===undefined?'MKT': positiontype)==='MKT'?'Completed':
            ((positionsidetype==='BUY'?'SELL':'BUY').toLowerCase()==='buy'?
            (parseFloat(calculateOrderPrice(getSetting(positioninstrumentname, positionexpirydate)?.defaultLMTPerCentage,positiontype,positionsidetype,ltp))>=parseFloat(ltp)?'Completed':'Pending'):
            (parseFloat(calculateOrderPrice(getSetting(positioninstrumentname, positionexpirydate)?.defaultLMTPerCentage,positiontype,positionsidetype,ltp))<=parseFloat(ltp)?'Completed':'Pending'))
            ),
            firstInInstrumentToken:firstInInstrumentToken.toString(),
            secondInInstrumentToken:secondInInstrumentToken.toString(),
     ...
     processExitAllPositionInBulk(scopedUpdatedRowsArray);
      ...
      const calculateOrderPrice=(defaultLMTPer,defaultOrderType,positionsidetype,ltp)=>{
        const orderprice=(
            defaultOrderType===undefined?'MKT': defaultOrderType)==='MKT'? ltp:
              (parseFloat(defaultLMTPer)>0?
              (positionsidetype==='BUY'?'SELL':'BUY').toLowerCase()==='buy'?
    ...
    let orderprice=(
      positiontype===undefined?'MKT': positiontype)==='MKT'? dataInfo.ltp:
        ...
        let data={
          ...
          let newSecondOutInstrumentToken=Constant.GetStrikeToken(globleOptionChainList,dataInfo.positioninstrumentname,dataInfo.positionexpirydate,newSecondOutStrike,temptype)
          let newFirstInExchangeToken=Constant.GetStrikeExchangeToken(globleOptionChainList,dataInfo.positioninstrumentname,dataInfo.positionexpirydate,newFirstInStrike,temptype);
          ...
          message=dataInfo.moveinoutqty.toString()+" Qty - Position "+dataInfo.positioninstrumentname+" "+dataInfo.strikeprice+" "+dataInfo.positionordertype+
          " moved to "+dataInfo.positioninstrumentname+" "+currentStrike+" "+dataInfo.positionordertype
      ...
      positiontype===undefined?'MKT': positiontype)==='MKT'? dataInfo.ltp:
        ...
        let data={
          ...
          let dataNew={
            ...
            nooforderlot:dataInfo.moveinoutqty.toString(),
            maxorderqty:getSetting(dataInfo.positioninstrumentname, dataInfo.positionexpirydate).defaultSliceQty.toString(),
            orderprice:((positiontype===undefined?'MKT': positiontype)==='MKT'?currentltp.toString():
            (dataInfo.positionsidetype.toLowerCase()==='buy'?
            (parseFloat(orderpriceNew)>=parseFloat(currentltp)?currentltp.toString(): orderpriceNew.toString()):
            (parseFloat(orderpriceNew)<=parseFloat(currentltp)?currentltp.toString(): orderpriceNew.toString()))
            ),
            tradermode:globleSelectedTradingType,
            orderidbybroker:"" ,
            ...
            orderstatus:((positiontype===undefined?'MKT': positiontype)==='MKT'?'Completed':
            (dataInfo.positionsidetype.toLowerCase()==='buy'?
            (parseFloat(orderpriceNew)>=parseFloat(currentltp)?'Completed':'Pending'):
            (parseFloat(orderpriceNew)<=parseFloat(currentltp)?'Completed':'Pending'))
            ),
            firstInInstrumentToken:newFirstInInstrumentToken.toString(),
            secondInInstrumentToken:newSecondInInstrumentToken.toString(),
                                        ...
                                        <CardHeader className="border-0">
                                            ...
                                            <Col xl="2"  xs="6" style={{"height":"25px","lineHeight":"20px"}} >
                                                                ...
                                                                <label
                                                                    ...
                                                                    <span className={(mltUnrealized>0? ' text-success': (mltUnrealized<0? 'text-danger': 'text-data-secondary'))+" m-1 font-14px  text-center"}> 
                                                                              {mltUnrealized>0?'+':''}{ Constant.CurrencyFormat(mltUnrealized)}
                                                                    </span>
                                            </Col>
                                            <Col xl="1" xs="4" className='stoplosstarget'>
                                                                    <fieldset className='border' onClick={()=>{
                                                                                         setSLEdit(true)
                                                                                    }}>
                                                                    ...
                                                                    <label className="float-right">
                                                                         {
                                                                                ...
                                                                                onChange={(e)=>{updateGlobalStopLoss(e.target.value)}}
                                                                            />
                                                                    }
                                                                    </fieldset>      
                                            </Col>
                                            ...
                                            <fieldset className='border' onClick={()=>{
                                                            ...
                                                            {!tpEdit?
                                                                                    <label className="float-right">                                                                                            
                                                                                        {(parseFloat(globalTP)>0? Constant.CurrencyFormat(globalTP.toString()):'---')}
                                                                                        </label> :
                                                                                        <Input
                                                                                        className="form-control-alternative text-right"
                                                                                        id="input-postal-code"
                                                                                        ...
                                                                                        onKeyDown={handleKeyDown}
                                                                                        value={globalTP}
                                                                                        onChange={(e)=>{updateGlobalTP(e.target.value)}}
                                                                                    />
                                                                                }
                                                    ...
                                                    <Table className="align-items-center">
                                                        <thead className="thead-light">
                                                            <tr className="text-center">
                                                                <th scope="col" style={{width:"3%"}}>Side</th>   
                                                                <th scope="col" style={{width:"5%"}}>Instrument</th> 
                                                                <th scope="col" style={{width:"3%"}}>Product</th>  
                                                                <th scope="col" style={{width:"5%"}}>LOT</th>                                                                  
                                                                <th scope="col" style={{width:"4%"}}>Avg Price</th>   
                                                                <th scope="col" style={{width:"3%"}}>LTP</th>  
                                                                <th scope="col" style={{width:"5%"}}>P&L</th>  
                                                                <th scope="col" style={{width:"4%"}}>Trailing</th>  
                                                                 <th scope="col" style={{width:"4%"}}>Target</th> 
                                                                <th scope="col" style={{width:"4%"}}>StopLoss</th>  
                                                                <th scope="col" colSpan={3}>Move IN/AWAY</th>
                                                                <th scope="col" style={{width:"9%"}}>ADD</th>   
                                                                <th scope="col" style={{width:"9%"}}>Exit</th>  
                                                                <th scope="col" style={{width:"5%"}}>Reverse</th> 
                                                            </tr>
                                                         </thead>
                                                         <tbody>
                                                            {
                                                                ...
                                                                filterOrderPosition!==undefined?(
                                                                    ...
                                                                    <tr key={index}>
                                                                        ...
                                                                        <span className={ dataInfo.positionsidetype.toLowerCase()==='buy'?'text-success text-bold buy-light':'text-danger text-bold sell-light'}>
  ...
  dataInfo.strikeprice === "0"
    ...
    : (
      <>
         <strong> {dataInfo.positioninstrumentname}  </strong> { Constant.ConvertShortDate(dataInfo.positionexpirydate)}   {dataInfo.strikeprice} {dataInfo.positionordertype}                                                                        
      </>
    )
}
                                                                        ...
                                                                        <td className='text-center'>
                                                                        <span className={ dataInfo.positionproductname.toLowerCase()==='mis'?'text-product-mis text-bold buy-light':'text-product-nmrd text-bold sell-light'}>
                                                                        ...
                                                                        <td className='text-center'>
                                                                            {/* {dataInfo.positionnetqty} */}
                                                                                ...
                                                                                <legend align="right">{(parseInt(dataInfo.positionnetqty)<0?(-1)*dataInfo.positionnetqty:dataInfo.positionnetqty)}</legend>
                                                                        ...
                                                                        <td 
                                                                        className={parseFloat(dataInfo?.unrealisedpnl)>0?'text-success text-right':(parseFloat(dataInfo?.unrealisedpnl)<0?'text-danger text-right':'text-data-secondary text-right')}>
            ...
            : (parseFloat(dataInfo.unrealisedpnl) > 0
                ...
                : Constant.CurrencyFormat(dataInfo.unrealisedpnl))
                                                                        ...
                                                                        <td className='text-center' style={{width:"4%"}}>
                                                                          {dataInfo.positionordertype.toLowerCase()==='fut'?'':
                                                                             <div className='moveinout' style={{ display: 'flex', alignItems: 'center' }}>
                                                                                    <button className='btn btn-danger text-black  movein-light mr-1'  
                                                                                    disabled={parseFloat(dataInfo.secondInltp)>0 && dataInfo.moveinoutqty>0?false:true}
                                                                                    onClick={(e)=>handdleSecondPosition(dataInfo,'in')}>
                                                                                      <b>  {dataInfo.secondInStrike} </b>({dataInfo.secondInltp})
                                                                                    </button>
                                                                                    <button className='btn btn-danger text-black  movein-light mr-1' 
                                                                                    disabled={parseFloat(dataInfo.firstInltp)>0 && dataInfo.moveinoutqty>0?false:true}
                                                                                    onClick={(e)=>handdleFirstPosition(dataInfo,'in')}>
                                                                                    <b>  {dataInfo.firstInStrike}</b> ({dataInfo.firstInltp})
                                                                                    </button>
                                                                             </div>
                                                                           }
                                                                        </td>
                                                                        ...
                                                                        {dataInfo.positionordertype.toLowerCase()==='fut'?'---':
                                                                           <fieldset className='border'>
                                                                           <legend align="right">{dataInfo.moveinouttotalqty}</legend>
                                                                              <Input
                                                                                    className="form-control-alternative"
                                                                                    id="input-postal-code"
                                                                                    placeholder="Qty"
                                                                        ...
                                                                        <td className='text-center moveinout' style={{width:"4%"}}>
                                                                        {dataInfo.positionordertype.toLowerCase()==='fut'?'':
                                                                        <div className='moveinout' style={{ display: 'flex', alignItems: 'center' }}>
                                                                            <button className='btn btn-success moveout-light text-black  mr-1' 
                                                                            disabled={parseFloat(dataInfo.firstOutltp)>0 && dataInfo.moveinoutqty>0?false:true}
                                                                            onClick={(e)=>handdleFirstPosition(dataInfo,'out')}>
                                                                            <b> {dataInfo.firstOutStrike}</b> ({dataInfo.firstOutltp})
                                                                            </button>
                                                                            <button className='btn btn-success moveout-light text-black     ml-1'  
                                                                            disabled={parseFloat(dataInfo.secondOutltp)>0 && dataInfo.moveinoutqty>0?false:true}
                                                                            onClick={(e)=>handdleSecondPosition(dataInfo,'out')}>
                                                                                  <b>  {dataInfo.secondOutStrike}</b> ({dataInfo.secondOutltp})
                                                                            </button>
                                                                           </div> 
                                                                          }
                                                                        </td>
                                                                        <td className='text-center'>
                                                                            ...
                                                                            <button className={`btn hide btn-danger text-danger text-bold sell-light mr-0 ${parseFloat(dataInfo.ltp)<0 || dataInfo.ltp===undefined ? 'disabled' : ''}`}  onClick={()=>handdleAddExistQty(dataInfo,'exit')}
                                                                            disabled={parseFloat(dataInfo.ltp)<0 || dataInfo.ltp===undefined ?true:false}>
                                                                                <i className='fas fa-close'></i>
                                                                            </button>
                                                                            <fieldset className='border'>
                                                                           <legend align="right">{dataInfo.newaddtotalqty}</legend>
                                                                            <Input
                                                                                className="form-control-alternative"
                                                                                id="input-postal-code"
                                                                                placeholder="Qty"                                                                                
                                                                            ...
                                                                            <button 
                                                                            className={`btn btn-success buy-light text-success text-bold ml-1 ${!parseFloat(dataInfo.ltp) || !parseInt(dataInfo.newqty) ? 'disabled' : ''}`}
                                                                            onClick={()=>handdleAddExistQty(dataInfo,'add')}
                                                                            disabled={!parseFloat(dataInfo.ltp) || !parseInt(dataInfo.newqty)}
                                                                            >
                                                                        ...
                                                                        <td className='text-center'>
                                                                            ...
                                                                            <fieldset className='border'>
                                                                           <legend align="right">{dataInfo.exittotalqty}</legend>
                                                                            <Input
                                                                                className="form-control-alternative"
                                                                                id="input-postal-code"
                                                                                placeholder="Qty"
                                                                            ...
                                                                            <button className={`btn btn-danger text-danger text-bold sell-light ml-1 ${!parseFloat(dataInfo.ltp) || !parseInt(dataInfo.exitqty) ? 'disabled' : ''}`}
                                                                            
                                                                               disabled={!parseFloat(dataInfo.ltp) || !parseInt(dataInfo.exitqty)}
                                                                        ...
                                                                        <td className='text-center'>
                                                                            ...
                                                                            <button className='btn btn-warning ml-1'  onClick={()=>handdleReverseOrderExist(dataInfo)}>
                                                                            REVERSE
                                                                            </button>
