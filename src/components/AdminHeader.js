...
import {
  Container,DropdownMenu,DropdownItem,
  Row,
  Col} from "reactstrap";
...
const AdminHeader = () => {
 ...
 const getSymbolExpiry=async ()=>{
  ...
  if(data!=null){
    if(data.objExpiryData!=null){
      data.objExpiryData.map((cl)=>(
        symbol.push(cl.name)
      ));      
      CookiesConfig.setCookie("symbolList",JSON.stringify(symbol));
      CookiesConfig.setCookie("symbolExpiryData",JSON.stringify(data.objExpiryData)); 
          
    }
    if(data.objTokenData!=null){      
      CookiesConfig.setCookie("symbolSpotTokenList",JSON.stringify(data.objTokenData));
    }  
    setSymbolData();   
  }
}
 ...
 const getExpiryForSymbol=(symbolName)=>{ 
    debugger;
    let symbolExpiryData=JSON.parse(CookiesConfig.getCookie("symbolExpiryData"));
    if(symbolExpiryData!=null){   
            const objExpiryList=symbolExpiryData.find((expiry) => expiry.name === symbolName);
            let dsSpotTokenList=JSON.parse(CookiesConfig.getCookie("symbolSpotTokenList")); 
            if(dsSpotTokenList!=null){
                if(channelName?.length==0){    
                  let channelData=dsSpotTokenList.map((data)=>({instrumentToken:data.instrumentToken,tokenType:data.tokenType}));
...
const getStockIndex=()=>{  
    ...
    const centrifugeInstance = new Centrifuge('wss://stock-api.fnotrader.com/connection/websocket');
          ...
          channel.on('publish', (data) => {            
            if(data.data!=null){ 
                ...
                setIndexData((previousData)=>{                    
                    const index = previousData.findIndex((dataToken) => dataToken.token === parseInt(cName.instrumentToken) && dataToken.tokenType===cName.tokenType);
                    if (index !== -1) {
...
useEffect(()=>{  
  if(stockSymbolInformation.length>0)   {   
    if(symbolSelect=='') {             
      ...
      CookiesConfig.setCookie("currentStockSymbol",stockSymbolInformation[0]);      
...
const handleTradingTypeData = (e) => {
  ...
  useEffect(()=>{   
    if(clientSelect!=="" && symbolSelect!=="" && expityvalue!=="" && tradingTypeSelect!==""){              
          ...
          <div className="header" style={{backgroundColor: "#FFFFFF"}}>
            <Container fluid>
              <div className="header-body">
                ...
                <Row className="py-1">                
                   ...
                   <Col xl="2"  xs="12">
                        <Row>
                              ...
                              <Col xl="6" xs="6">
                                <Select options={optionsClient} value={clientSelect} styles={customStyles}
                   ...
                   <Col xl="3"  xs="12">
                        <Row>
                        <Col xl="4" xs="4">
                                <Select options={optionsTradingType} value={tradingTypeSelect} styles={customStyles}
                                onChange={handleTradingTypeData}/>
                              </Col>
                          <Col xl="4" xs="4">
                                    <Select options={stockSymbolInformation} value={symbolSelect} onChange={handleSymbolChange} styles={customStyles} />
                          </Col>
                          <Col xl="4" xs="4">
                                  <Select options={expityData}  value={expityvalue}   styles={customStyles}  onChange={handleExpityData}/>
                            </Col>
                      ...
                      <Row>
                            <Col xl="5" xs="5">
                                  <Row>
                                      <Col xl="12"  className='' style={{textAlign:"center"}}>                                          
                                          <label  className="form-control-label mr-2 font-9px">
                                              SPOT    
                                           </label>
                                           <label  className={'font-11px form-control-label  mr-2 '+(parseFloat(currentStockLTP)>=0? 'text-success currentstock':'text-danger currentstock')}>
                                             {parseFloat(currentStockIndex).toFixed(2)}
                                           </label>
                                           <label  className={'font-11px form-control-label  '+(parseFloat(currentStockLTP)>=0? 'text-success currentstock':'text-danger currentstock')}>
                                           {(parseFloat(currentStockLTP)>=0?  "+ " :  "" )}
                                          {parseFloat(currentStockLTP).toFixed(2)}
                                          ({(parseFloat(currentStockLTP)>=0?  "+ " :  "-" )}{parseFloat(currentStockLTPPercent).toFixed(2)}%)
                                           </label>
                                      </Col>
                            ...
                            <Row>
                                      <Col xl="12"  style={{textAlign:"center"}}>                                          
                                          <label  className="form-control-label  font-9px mr-2">
                                              FUTURE       
                                           </label>
                                           <label  className={"font-11px form-control-label  mr-2 "+(currentStockLTPFuture>=0? 'text-success currentstock':'text-danger currentstock')}>
                                                    {parseFloat(currentStockIndexFuture).toFixed(2)}
                                           </label>
                                           <label  className={"font-11px form-control-label "+(currentStockLTPFuture>=0? 'text-success currentstock':'text-danger currentstock')}>
                                           {(parseFloat(currentStockLTPFuture)>=0?  "+ " :  "" )}
                                          {parseFloat(currentStockLTPFuture).toFixed(2)} 
                                          ({(parseFloat(currentStockLTPFuture)>=0?  "+ " :  "-" )}{parseFloat(currentStockLTPPercentFuture).toFixed(2)}%)
                                           </label>
                                      </Col>
                                  </Row>
                            </Col>
                               ...
                               <Nav className="align-items-center d-none d-md-flex" navbar>
                                  <UncontrolledDropdown nav>
                                      <DropdownToggle className="pr-0" nav>
                                            <i className='fas fa-gear' style={{fontSize:"20px"}}></i>
                                      </DropdownToggle>
                ...
                <DropdownItem href="#pablo" onClick={(e) => handdleLogOut()}>
